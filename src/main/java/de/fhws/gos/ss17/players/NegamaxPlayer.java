  ...
  private Move getBestMove(Board board, PositionToken playerToken) throws GameException {
    ...
    for (Move move : validMoves) {
      while (maxer > 0) {
        ...
        int evaluationResult = -evaluateNegaMax(depth - 1, "", Integer.MIN_VALUE,
            Integer.MAX_VALUE,
            board, playerValue);
        board.undoMove(move);
        ...
        maxer--;
        //System.out.println("1 Blatt von Moves abgearbeitet");
      }
    }
    System.out.println(
        "Best Move is FROM " + bestMove.getFromId() + "TO " + bestMove.getToId() + "REM " + bestMove
            .getRemoveId());
    return bestMove;
  }

  public int evaluateNegaMax(int depth, String indent, int alpha, int beta, Board board,
      int playerValue)
